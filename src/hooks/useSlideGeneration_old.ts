...
export function useSlideGeneration() {
  ...
  const generateSlides = async (options: UseSlideGenerationOptions) => {
    try {
      ...
      // Step 2: Repository Analysis (using singleton instance)
      ...
      // Step 3: Story Generation (using singleton instance)
      let storyStructure = storyGenerator.generateStory(repositoryData)

      // Step 4: Skip LLM Enhancement for now
      if (options.enableLLMEnhancement) {
        setProgress({
          stage: 'story',
          progress: 55,
          message: 'AIによるコンテンツ強化は現在無効です...'
        })
